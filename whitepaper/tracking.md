...
| 16 | Encode Disaggregation (Multimodal) | `guides/multimodal-serving/e-disaggregation` | ➖ | | **Excluded from whitepaper** — investigated, upstream "Supported Hardware Backends" table lists NVIDIA GPU (vLLM) only, no Intel XPU entry or substitution path. No chapter written; re-check if upstream adds a `modelserver/xpu/` overlay |
