...
An [Intel XPU variant](./xpu.md) is also available. It uses ModelExpress with host-staged TCP and
Level Zero copies. XPU-direct RDMA is disabled until a content-dependent B60 device-memory
corruption is fixed and passes the documented checksum gate.
...
ModelExpress](./xpu.md) for the Qwen3-0.6B, Intel GPU DRA, and `tcp,ze_copy` configuration.
...
* [Running ModelExpress on Intel XPU](./xpu.md): deploy the correctness-first B60 path and review
  the direct-RDMA safety gate.
