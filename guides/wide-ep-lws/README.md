...
> path: the shipped vLLM build for Intel XPU rejects `--enable-eplb` outright,
> and an unofficial from-source build that removes that restriction still hit
> an unresolved device-level crash during cross-node testing. Treat EPLB as
> unsupported on Intel XPU for now.
